-- clariq-demo only. Never apply to production.
--
-- seed_demo(): builds the demonstration fleet through create_container() and the
-- event trigger, exactly as production data is built. Deterministic (setseed 0.42),
-- so every reset produces the same fleet. Body is the 25 Aug 2026 seed script with
-- two changes: the tenant and the actor are looked up rather than hard-coded, and
-- the Clarq test customer, Meremere site and 24 NEW containers are appended.
--
-- reset_demo(): removes every business row, restarts the sequences and re-runs
-- seed_demo(). Keeps tenant, roles, reference lists, container types (the seed
-- re-creates the four HDPE types; TYPE-AUDIT-UNKNOWN is kept), users, invites and the
-- Ask Clariq corpus.
--
-- The seeder actor is a permanent app_users row with no auth.users row behind it, so
-- it can never sign in. It exists so seeded events have an attributable actor that
-- is not a real person.
--
-- Both functions are owner-only. They are run through the Supabase connector or the
-- SQL editor, never from the app.

insert into app_users (id, tenant_id, role_id, display_name, email, can_authorise, active)
select '00000000-0000-4000-8000-00000000c1a9', t.id, r.id, 'Clariq Demo Seeder', 'seeder@demo.clariq.nz', true, true
from tenants t, roles r where r.code = 'ADMIN'
...
declare
  t uuid := (select id from tenants order by created_at limit 1);
  actor uuid := '00000000-0000-4000-8000-00000000c1a9';  -- Clariq Demo Seeder (see header)
...
begin
  -- act as the seeder for this transaction (actor_has() inside the triggers needs it)
  perform set_config('request.jwt.claims', '{"sub":"00000000-0000-4000-8000-00000000c1a9","role":"authenticated"}', true);
  perform set_config('request.jwt.claim.sub', '00000000-0000-4000-8000-00000000c1a9', true);
  -- the seed back-dates CREATED events; that update is the only one ever made to an event
  ...
  -- Test records carried over from production before the 09-09-2026 purge:
  -- the Clarq trial customer, its Meremere site, and 24 NEW 5 L containers
  -- created in three label-printing batches (26, 27 and 29 Aug 2026).
  insert into customers (tenant_id, legal_name, trading_name, primary_contact, email, phone, account_status, deposit_arrangement, location_labels, created_by)
  values (t, 'Clarq', 'Clarq', 'Jay', 'jnf1306@gmail.com', '', 'ACTIVE', 'NONE',
          '{"preset":"GENERIC","faculty":"Division","building":"Building","room":"Room / Area","cabinet":"Storage unit"}'::jsonb, actor)
  returning id into c1;
  insert into sites (tenant_id, customer_id, name, address, created_by)
  values (t, c1, 'Meremere', '{"line1":""}'::jsonb, actor);
  for i in 1..24 loop
    t0 := (case when i <= 8 then timestamptz '2026-08-26 10:00+12'
                when i <= 16 then timestamptz '2026-08-27 10:00+12'
                else timestamptz '2026-08-29 10:00+12' end);
    c := create_container(t, ty5, 'Clariq', t0::date, null);
    update container_events set occurred_at = t0, recorded_at = t0 where container_id = c.id;
  end loop;
