-- 0041: two things the end-user home needs.
-- 1. container_types read for linked end users: v_site_inventory and the
--    overdue view inner-join types, so without this the rows vanish.
-- 2. v_site_inventory honours EMPTIED (0034): quantity on hand after an
--    EMPTIED since dispatch takes precedence over the as-dispatched figure,
--    and emptied_at is exposed so the register can say so.
create policy container_types_read_linked on public.container_types
