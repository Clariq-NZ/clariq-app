...
const SECTIONS: { title: string; steps: string[]; staff?: boolean }[] = [
  ...
  { staff: true, title: 'Record an action (fill, dispatch, return, wash, inspect)', steps: ['From the container card, tap the action.', 'Fill in only the fields shown; each is required for that action.', 'Tap the button at the bottom. The event is permanent. If you made a mistake, add a Note; an Admin can record an adjustment.'] },
  { staff: true, title: 'Set up a customer', steps: ['Menu, Customers, Add a customer. Legal name is enough to start.', 'Open the customer and add each site (a campus, depot or plant).', 'Open a site and add locations: faculty, building, room or lab, cabinet. Only fill what applies.', 'Menu, Products, to add the chemicals you expect to find or supply.'] },
  { staff: true, title: 'Print labels', steps: ['Menu, New containers and labels. Choose the type (use Audit unknown for containers Clariq does not own) and how many.', 'Download the PDF and print on Avery L7060 polyester sheets.', 'Each label is now a live container in status New, waiting to be bound on a walk.'] },
  { staff: true, title: 'Run an audit walk', steps: ['Menu, Audit. Choose the customer and site, enter the expected count if you have one, tap Start.', 'At each container: stick a label if it has none, tap Scan next container, take the photo, choose or add the location, choose the condition, choose the contents, tap Record sighting.', 'The location you chose stays selected for the next container, so a cabinet of twenty takes twenty scans and little else.', 'When the site is done, tap Close the walk. The reconciliation shows what was sighted, what was expected but not sighted, and what turned up somewhere else.'] },
  { staff: true, title: 'See what a customer sees', steps: ['Menu, View as a customer, choose the customer.', 'Today and the reports open with that customer locked on and staff actions hidden.', 'Tap the lock in the header to return to the staff view.'] },
...
export default function GuidePage() {
  ...
  const sections = SECTIONS.filter(s => !s.staff || !cv)
  return (
